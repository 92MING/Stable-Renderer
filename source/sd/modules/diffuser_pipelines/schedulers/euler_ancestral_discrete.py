...
class EulerDiscreteScheduler(SchedulerMixin, ConfigMixin):
    ...
    def __init__(
        ...
        disable_ancestral_at_last: bool = False,
        img2img: bool = False,
    ):
    ...
    def step(
        ...
        pred_original_sample = self.predict_original_sample(model_output, timestep, sample, generator)

        prev_sample = self.predict_previous_sample(pred_original_sample, model_output, timestep, sample, generator) 
    ...
    # Implements the TwoStepScheduler protocol
    ...
    def predict_previous_sample(self,
        ...
        use_ancestral = self.config.enable_ancestral_sampling
        if self.config.disable_ancestral_at_last and self.step_index == self.config.num_train_timesteps - 1:
            use_ancestral = False
        ...
        if use_ancestral:   # Add new random noise only if ancestral sampling is enabled            
