...
class Overlap:
    ...
    def __call__(
        ...
        **kwargs,
        ...
        DefaultLogger.info(f"Scheduler: alpha: {alpha} | corr_map_decay: {corr_map_decay:.2f} | kernel_radius: {kernel_radius} | timestep: {timestep:.2f}")
        ...
        toc = time.time()
        DefaultLogger.debug(f"Overlap cost: {toc - tic:.2f}s in total | {(toc-tic)/num_frames:.2f}s per frame") if self.verbose else ...
        DefaultLogger.debug(f"Vertex appeared once: {len_1_vertex_count * 100 / max(len(corr_map), 1) :.2f}% | Average trace length: {avg_trace_length / max(len(corr_map), 1) :.2f} | Index decayed: {index_decay_count}")
...
class ResizeOverlap(Overlap):
    ...
    def __call__(
        ...
        **kwargs
        ...
        DefaultLogger.debug(f"Resize scale factor: {screen_h / frame_h:.2f} | Overlap ratio: {100 * self.overlap_rate(ovlp_seq, threshold=0):.2f}%")
