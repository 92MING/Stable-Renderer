...
def calc_map_mean_std(feat:torch.Tensor, eps=1e-5, mode: Literal['NCHW', 'NHWC']='NCHW'):
    '''
...
def adaptive_instance_normalization(content_feat: torch.Tensor, style_feat: torch.Tensor, eps=1e-5, mode: Literal['NCHW', 'NHWC']='NCHW'):
    ...
    style_mean, style_std = calc_map_mean_std(style_feat, eps, mode)
    content_mean, content_std = calc_map_mean_std(content_feat, eps, mode)
