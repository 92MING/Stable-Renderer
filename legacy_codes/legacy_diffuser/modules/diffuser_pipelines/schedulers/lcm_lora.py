...
class LCMScheduler(SchedulerMixin, ConfigMixin):
    ...
    def __init__(
        ...
        disable_ancestral_at_last=False,
        img2img: bool = False,
    ):
    ...
    def lcm_origin_timesteps(self):
        ...
        lcm_origin_timesteps = np.asarray(list(range(1, int(original_steps * 1.0) + 1))) * k - 1
        
        return lcm_origin_timesteps
    ...
    def set_timesteps(
        ...
        if not self.config.img2img:
            strength = 1.0  # for non-img2img pipelines, no need `strength` parameter, so set it to 1.0
        ...
        if timesteps is not None:
            ...
            origin_time_steps = list(self.lcm_origin_timesteps)
            origin_time_steps.sort(reverse=True)
            init_origin_timestep = min(int(original_steps * strength), original_steps)
            origin_t_start = max(original_steps - init_origin_timestep, 0)
            first_origin_timestep = origin_time_steps[origin_t_start * self.order]
            
            remove_index = None
            for i in range(len(timesteps)):
                if timesteps[i] <= first_origin_timestep:
                    remove_index = i - 1
                    break
            if remove_index is not None and remove_index > 0:
                timesteps = timesteps[remove_index:]
    ...
    def predict_previous_sample(self,
        ...
        if self.config.enable_ancestral_sampling and not (self.config.disable_ancestral_at_last and self.step_index == self.num_inference_steps - 1):
            noise = randn_tensor(
