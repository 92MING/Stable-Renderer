...
import glm
from sample.tutorial.example_utils.glutWindow import GlutWindow
from sample.tutorial.example_utils.shaderLoader import Shader
import numpy as np
...
class Tu01Win(GlutWindow):
	...
	def calc_MVP(self,width=1920,height=1080):
		...
		self.context.MVP = Matrix.Perspective(45, width/height, 0.1, 1000) * Matrix.LookAt([4,3,-3],[0,0,0],Vector.up()) * Matrix.Identity(4)
		#self.context.MVP = np.ascontiguousarray(self.context.MVP.flatten(), np.float32)
		#self.context.MVP = np.array(self.context.MVP.flatten(), np.float32)
		print(self.context.MVP)
	...
	def ogl_draw(self):
		...
		glUniformMatrix4fv(self.context.MVP_ID, 1, GL_TRUE, self.context.MVP.data)
